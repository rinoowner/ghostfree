...
class FloatingWindowService : Service() {
    ...
    private lateinit var displayIpPort: TextView
    ...
    override fun onCreate() {
        ...
        terminalLog = floatingView.findViewById(R.id.terminal_log)
        displayIpPort = floatingView.findViewById(R.id.display_ip_port)
        ...
        syncJob = CoroutineScope(Dispatchers.Main).launch {
            while (true) {
                ...
                if (currentTime < attackEndTime) {
                    ...
                    attackTimeLabel.text = "Attack Time: ${remainingSec}s"
                    
                    if (ip != null && port != null) {
                        displayIp.text = "IP: $ip"
                        displayPort.text = "PORT: $port"
                        displayIpPort.text = "$ip:$port"
                        terminalLog.text = "TARGET: $ip:$port\nREMAINING: ${remainingSec}s\nSTATUS: OVERLOAD_ACTIVE"
                        terminalLog.visibility = View.VISIBLE
                    }
                } else {
                    if (isSearching) {
                        if (ip != null && port != null) {
                        ...
                        if (ip != null && port != null) {
                            ...
                            displayIp.text = "IP:                "
                            displayPort.text = "PORT:            "
                            displayIpPort.text = "[ IDLE ]"
                        }
                    }
                    terminalLog.visibility = View.GONE
                }
